...
import { LuTrash, LuUpload, LuUser } from "../utils/icons";
      ...
      {!image ? (
        <div className="relative flex items-center justify-center w-20 h-20 rounded-full cursor-pointer bg-sky-50">
          ...
          <button
            type="button"
            onClick={onChoseFile}
            className="absolute flex items-center justify-center w-8 h-8 text-white rounded-full cursor-pointer bg-linear-to-r from-sky-500 to-cyan-500 -bottom-1 -right-1"
          >
            <LuUpload />
          </button>
        </div>
