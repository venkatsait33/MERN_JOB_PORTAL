...
import { MdOutlineHomeWork, MdWebAsset } from 'react-icons/md';
                            ...
                            return (
                                <div key={job._id} className='shadow-xl card '>
                                    <div className='flex flex-col gap-2 card-body'>
                                        <Link to={`/job/description/${job._id}`} className='my-1 mt-4 font-semibold link'>Job Title : {job.title}</Link>
                                        <p className='flex items-center gap-2 my-1 font-semibold'><CiLocationOn />Location: <span className='pl-4 font-normal ' > {job?.location || "N/A"}</span></p>
                                        <p className='flex items-center gap-2 my-1 font-semibold'><FaIndianRupeeSign />Salary: <span className='pl-4 font-normal '> {job?.salary || "N/A"}</span></p>
                                        <p className='my-1 font-semibold'>Posted Date: <span className='pl-4 font-normal ' >{job?.createdAt?.split('T')[0]}</span></p>
                                    </div>
                                </div>
                            )
